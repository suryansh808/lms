...
const AcceptedApplication = () => {
  ...
  const handleEdit=(user)=>{
    const isConfirmed = window.confirm("Are you sure you want to edit the user details?");
   if(isConfirmed){
  ...
  const handleActiveNow = async (id) => {
    const isConfirmed = window.confirm("Are you sure you want to inactive this user?");
     if(isConfirmed){
      try {
              ...
              users.length > 0 ? (
                users?.map((user, index) => (
                  <tr key={user._id}>
                    ...
                    <td>{user.status}</td>
                    <td>
                      <button onClick={()=> handleActiveNow(user._id)}>Inactive now</button>
