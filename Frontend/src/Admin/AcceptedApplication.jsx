...
const AcceptedApplication = () => {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      const enrollmentsMap = new Map(enrollmentsResponse.data.map(e => [
        e.email, 
        { 
          program: e.program, 
  ...
  const fetchComponents = useCallback(async (userId) => {
    try {
      const response = await axios.get(`${API}/user-components`, { params: { userId } });
  ...
  const handleToggleComponent = useCallback(async (userId, component, status) => {
        ...
        <div className="flex gap-2">
          <button 
            ...
            onClick={async () => {
              ...
              try {
                await axios.put(`${API}/user-components/${userId}`, { component, status });
                const components = await fetchComponents(userId);
                setUsers(prev => prev.map(u => 
                  u._id === userId ? { ...u, components, isLoadingComponent: false } : u
                ));
  ...
  const filteredUsers = useMemo(() => {
    return users.filter(user => {
      const matchesSearch = !searchQuery || 
        (programFilter && user.program !== programFilter ? false : 
  ...
  const handleInactivate = useCallback(async (id) => {
      ...
      <tr key={user._id}>
        ...
        <td>
          {user.components ? (
            <>
              <button
                className={`px-2 py-1 rounded mr-1 ${user.components?.atschecker ? "bg-green-500 text-white" : "bg-gray-300"} ${user.isLoadingComponent ? 'opacity-50 cursor-not-allowed' : ''}`}
              ...
              <button
                className={`px-2 py-1 rounded ${!user.components?.atschecker ? "bg-red-500 text-white" : "bg-gray-300"} ${user.isLoadingComponent ? 'opacity-50 cursor-not-allowed' : ''}`}
        ...
        <td>
          {user.components ? (
            <>
              <button
                className={`px-2 py-1 rounded mr-1 ${user.components?.jobboard ? "bg-green-500 text-white" : "bg-gray-300"} ${user.isLoadingComponent ? 'opacity-50 cursor-not-allowed' : ''}`}
              ...
              <button
                className={`px-2 py-1 rounded ${!user.components?.jobboard ? "bg-red-500 text-white" : "bg-gray-300"} ${user.isLoadingComponent ? 'opacity-50 cursor-not-allowed' : ''}`}
        ...
        <td>
          {user.components ? (
            <>
              <button
                className={`px-2 py-1 rounded mr-1 ${user.components?.myjob ? "bg-green-500 text-white" : "bg-gray-300"} ${user.isLoadingComponent ? 'opacity-50 cursor-not-allowed' : ''}`}
              ...
              <button
                className={`px-2 py-1 rounded ${!user.components?.myjob ? "bg-red-500 text-white" : "bg-gray-300"} ${user.isLoadingComponent ? 'opacity-50 cursor-not-allowed' : ''}`}
        ...
        <td>
          {user.components ? (
            <>
              <button
                className={`px-2 py-1 rounded mr-1 ${user.components?.mockinterview ? "bg-green-500 text-white" : "bg-gray-300"} ${user.isLoadingComponent ? 'opacity-50 cursor-not-allowed' : ''}`}
              ...
              <button
                className={`px-2 py-1 rounded ${!user.components?.mockinterview ? "bg-red-500 text-white" : "bg-gray-300"} ${user.isLoadingComponent ? 'opacity-50 cursor-not-allowed' : ''}`}
        ...
        <td>
          {user.components ? (
            <>
              <button
                className={`px-2 py-1 rounded mr-1 ${user.components?.exercise ? "bg-green-500 text-white" : "bg-gray-300"} ${user.isLoadingComponent ? 'opacity-50 cursor-not-allowed' : ''}`}
              ...
              <button
                className={`px-2 py-1 rounded ${!user.components?.exercise ? "bg-red-500 text-white" : "bg-gray-300"} ${user.isLoadingComponent ? 'opacity-50 cursor-not-allowed' : ''}`}
        ...
        <div className="coursetable">
          ...
          <div className="flex items-center justify-center gap-1">
            <div className="relative group inline-block">
              ...
              <div className="absolute left-1/2 -translate-x-1/2 bottom-full z-[9999] mb-2 hidden w-max bg-gray-800 text-white text-sm rounded-md py-2 px-3 group-hover:block">
                {programFilter ? 
                  `Searching within ${PROGRAM_OPTIONS.find(opt => opt.value === programFilter)?.label} program` : 
            ...
            <div className="relative w-full">
              <input
                type="text"
                placeholder={programFilter ? 
                  `Search within ${PROGRAM_OPTIONS.find(opt => opt.value === programFilter)?.label}...` : 
           ...
           <label>
              <input
                ...
                onChange={e => { setShowFullyPaidOnly(e.target.checked); setCurrentPage(1); }}
            ...
            <select
              value={programFilter}
              onChange={e => { setProgramFilter(e.target.value); setCurrentPage(1); setSearchQuery(""); }}
              ...
              {paginatedUsers.length > 0 ? (
                ...
                <tr>
                  <td colSpan="11">
                    ...
                    {programFilter && ` in ${PROGRAM_OPTIONS.find(opt => opt.value === programFilter)?.label}`}
                  </td>
                </tr>
              )}
            </tbody>
          ...
          {filteredUsers.length > ITEMS_PER_PAGE && (
            <section className="flex items-center justify-center gap-5 mt-2">
              ...
              <button
                onClick={() => setCurrentPage(prev => prev < Math.ceil(filteredUsers.length / ITEMS_PER_PAGE) ? prev + 1 : prev)}
