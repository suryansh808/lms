...
// const authMiddleware = require("../middleware/UserAuth"); // Assuming same middleware
...
//     let coordinators;
//     if (coordinatorId) {
//       // Fetch specific coordinator by coordinatorId
//       coordinators = await PlacementCoordinator.findById(coordinatorId);
//       if (!coordinators) {
//         return res.status(404).json({ message: "Coordinator not found for the given coordinatorId" });
//       }
//     } else {
//       // Fetch all coordinators if no coordinatorId is provided
//       coordinators = await PlacementCoordinator.find().sort({ _id: -1 });
//     }
...
// // Protected route for Placement Coordinator Dashboard
// router.get("/pcdashboard", authMiddleware, (req, res) => {
//   res.status(200).json({ message: "Welcome to the Placement Coordinator Dashboard!" });
// });
